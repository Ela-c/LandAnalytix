...
export default function UploadParcels({ onAddParcels }: UploadParcelsProps) {
	...
	const handleDrop: React.DragEventHandler<HTMLLabelElement> = async (
		event,
	) => {
	...
	const handleDragEnter: React.DragEventHandler<HTMLLabelElement> = (
		event,
	) => {
		event.preventDefault();
	...
	return (
		<>
			<div className="flex items-center justify-between mb-2 text-sm text-neutral-300">
			...
			>
				...
				<span>
					{isDragging
						? "Release to upload"
						: "Drop or select a GeoJSON file"}
				</span>
			</label>
			{status && (
				<div className="mt-2 text-xs text-neutral-400">{status}</div>
			)}
		</>
	);
}
