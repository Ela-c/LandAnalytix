...
export default function SearchPanel({
    ...
    const parcelResults = useMemo(() => {
        ...
        return parcels
            .filter((parcel) => matchesSearch(`${parcel.name} ${parcel.id}`, term))
            .slice(0, 20);
    }, [parcels, searchTerm]);
    ...
    const results: SearchResult[] = useMemo(() => {
        const lotResults = parcelResults.map((parcel) => ({
            ...
            sublabel: (parcel.properties?.address as string) ?? "No address available",
        }));
    ...
    const statusMessage = addressLoading
        ? "Searching..."
        : geocodeStatus ??
          (searchTerm.trim() && !hasResults ? "No matches. Try another term." : null);
    ...
    const showDropdown = isFocused && (addressResults.length > 0 || searchTerm.trim().length > 0);

    return (
        <div className="relative pointer-events-auto">
            <div
                className={`flex items-center gap-2 rounded-lg bg-white px-3 py-2.5 transition-all ${
                    isFocused ? "border-2 border-blue-500 " : "border border-neutral-700"
                }`}
            >
